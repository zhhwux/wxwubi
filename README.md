...
- （三）五笔造词
    ...
    3. 个人自定义词典（不参与组句）：在custom_phrase/user.txt按照：每行1条文本 的格式编辑，即自动生成五笔简词，作为官方五笔词的补充
- （四）候选词手动调序
    - 候选词排序调整：选中候选词后 ，按 Ctrl + j 向左移动；按 Ctrl + k 向右移动；按 Ctrl + p 置顶；按 Ctrl + l 清除调序信息。（调序信息储存在lua文件夹的sequence.userdb文件夹，把lua/sequence.txt移走就会自动导出最新调频记录到sequence.txt；放回去部署就会导入）
- （五）输入功能符号
    - 万象五笔方案新增或改动交互
...
## 更新日志
- 9.1：修复了选字时的首选字与平时不一致的问题、修复了\成对符号参与生成简词的问题、主题（）改为()、修复了两个报错、删改了若干报错字词、z历史调整为3次
- 8.20：启用了万象的选字造词功能和快符功能、为清屏上屏顶屏增加可开关配置、调整了主题、增加z 或【 或】的重复上屏方式、修复了部分部首未被形单开关控制的问题、调整rear_penalty为-18、修复拼音方案部件拆字失效的问题、修复/day被统计字数的问题，词库更新
- 8.1：调整了拆分与字体、新增4个文件处理功能、设定custom_phrase/user.txt文件内的带英文候选词在不开英文开关时也会固定在首选、修复了万象拼音自带拆分失效的问题、修复了中文输入法闪退的问题
- 7.27：修复文件简词3字词编码生成错误的问题、设定无custom_phrase/user.txt时自动生成空文件、新增Z前缀文件检索与管理功能、恢复快符a、调整了英文开关逻辑（ctrl+m开启）、跟随万象更新（词库路径更改、移除预测功能、拼音类型切换指令）
- 7.21：新增根据中文词库批量生成五笔词的功能（在custom_phrase/user.txt放入词库，格式是每行一条中文短语。当不加编码时，重新部署或输入/wjjc即可生成五笔词写入文件，由形词开关控制；已有编码和权重不会覆盖，输入/zyj会把“文件简词”转为“永久简词”即固定在次选的简词，输入/zwj则相反）、修复了主翻译器词库无法加入简词的问题
- 7.17：制作了一份无须安装字体既可显示大部分字根的五笔拆分表、将内置字体更换为 86字根拆解.ttf 兼容两种拆分显示
