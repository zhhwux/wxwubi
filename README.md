...
## 方案简介
[万象五笔](https://github.com/zhhwux/wxzhh)是基于[万象方案](https://github.com/amzxyz/rime_wanxiang_pinyin)改造的 Rime 输入方案。结合了五笔强大的离散效果和万象优质词库及模型对整句的优化
...
3. 五笔二三整句：五笔句/单/词交替上屏（一二根字 2 码 / 三四根字 3 码）
4. 五笔原码整句：最短首选编码，1码字后加'单引后缀（分词符改为_）
4. 九键五笔整句
---

## 核心交互
- （一）快捷键
    - `Control+a`（五笔句开关）/ `Control+s`（五笔词开关）/ `Control+d`（五笔单开关）/ `Control+z`（显示拆分）/ `Control+y`（显示注音）/ `Control+m`（中英混输开关）/ `Control+r`（预测开关）/ `Control+y`（音码开关）
- （二）拼音反查
    1. 万象五笔的拼音反查是 '单引号前缀 或 反引号后缀
    2. 默认小鹤+五笔辅（可以使用指令更改拼音类型，输入'/zrm'，快速切换为自然码双拼， /flypy→小鹤双拼 /mspy→微软双拼 /zrm→自然码 /sogou→搜狗双拼 /abc→智能ABC /ziguang→紫光双拼 /pyjj→拼音加加 /gbpy→国标双拼 /lxsq→乱序17 
       /pinyin→全拼
       也可以在wanxiang_pro.custom.yaml更改）
    3. 拼音注音滤镜开关 ctrl+y
- （三）五笔造词
    1. 选字造词（保存在custom_phrase/zaoci.txt）
    2. 五笔4码临时简词功能：只要最近上屏了一段文本(无论上屏文本是用五笔还是拼音打出来的)，就可以在次选用4码五笔词的格式打出这段文字 ，接下来确认上屏的自造词将会被永久化储存在custom_phrase/jianci.lua文件中
    3. 个人自定义词典（不参与组句）：在custom_phrase/user.txt按照：每行1条文本 的格式编辑，部署即自动生成五笔简词，作为官五笔词的补充
- （四）候选词手动调序
...
## 更新日志
- 2.7：合并了两个小字集滤镜、为注音和英文功能添加了配置项开关、优化了文件管理功能、修复了不开五笔句时反查也会自动上屏的问题、调整了拼音方案的反查、符号和快符、更随万象更新
- 2.5：进一步优化了流畅度，上屏速度达到秃五笔水平、修复了简词造词错误的问题，并为简词在4字以上时的造词规则添加了一个取前4码的模式选择、增加了3码时五笔词/五笔字词的completion功能和开关、增加了一个字词方案(未启用)、移除了五笔方案的ctrl+y拼音开关、调换了注音与音码的快捷键、调换了五笔单与五笔句的快捷键、调整了英文候选词、取消了在未查到长词拼音时的拼接行为
- 1.21：降低英文词权重为-1，使其不会异常排在拼音词前面、调整了其它版本五笔
- 1.20：修复/后缀快符失效的问题、修改拼音切换指令lua使五笔方案时也可生效指令、调整了注音来源词典、微调了主题
...
- 8.20：启用了万象的选字造词功能和快符功能、为清屏上屏顶屏增加可开关配置、调整了主题、增加z 或【 或】的秃五笔的重复上屏方式、修复了部分部首未被五笔单开关控制的问题、调整rear_penalty为-18、修复拼音方案部件拆字失效的问题、修复/day被统计字数的问题，词库更新
...
- 7.27：修复文件简词3字词编码生成错误的问题、设定无custom_phrase/user.txt时自动生成空文件、新增Z前缀文件检索与管理功能、恢复快符a、调整了英文开关逻辑（ctrl+m开启）、增加五笔英文混合词汇、修复了若干原码句词库问题、跟随万象更新（词库路径更改、移除预测功能、拼音类型切换指令）
- 7.21：新增根据中文词库批量生成五笔词的功能（在custom_phrase/user.txt放入词库，格式是每行一条中文短语。当不加编码时，重新部署或输入/wjjc即可生成五笔词写入文件，由五笔词开关控制；已有编码和权重不会覆盖，输入/zyj会把“文件简词”转为“永久简词”即固定在次选的简词，输入/zwj则相反）、原码句的一简后缀由_改为'英文单引，分词符改为_，取消1简字有词可以打2码的设计、修复了主翻译器词库无法加入简词和原码句首码加'时无候选的问题
